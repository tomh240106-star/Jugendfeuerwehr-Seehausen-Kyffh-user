...
import '../services/alarm_native_service.dart';
...
class _AlarmScreenState extends State<AlarmScreen> {
  ...
  Future<void> _loadResponses() async {
    ...
    try {
      ...
      if (_isTrainer) {
        final recipientRows = await _supabase
            .from('alarm_recipients')
            .select('user_id')
            .eq('alarm_id', alarm['id']);

        final recipientIds = List<Map<String, dynamic>>.from(recipientRows)
  ...
  Future<void> _createAlarm() async {
    ...
    final eligibleRows = await _supabase
        .from('profiles')
        .select('id,first_name,last_name,role')
    ...
    final result = await showDialog<Map<String, dynamic>>(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) {
          ...
          String roleOf(Map<String, dynamic> p) {
            return p['role']?.toString() == 'ausbilder'
                ? 'Ausbilder'
                : 'Jugendmitglied';
          }
                ...
                onPressed: () {
                  ...
                  Navigator.pop(
                    context,
                    {
                      ...
                      'recipient_ids': selectedIds.toList(),
                    },
                  );
                },
    ...
    final recipientIds =
        List<String>.from(result['recipient_ids'] as List<dynamic>);

    if (recipientIds.isEmpty) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Bitte mindestens einen Empfänger auswählen.'),
        ),
      );
      return;
    }
    ...
    Map<String, dynamic>? createdAlarm;
    ...
    try {
      ...
      createdAlarm = Map<String, dynamic>.from(inserted);

      await _supabase.from('alarm_recipients').insert(
            recipientIds
                .map(
                  (id) => {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Alarm wurde an ${recipientIds.length} ausgewählte Empfänger gesendet.',
          ),
        ),
      );
    } catch (e) {
  ...
  Future<void> _respond(String response) async {
    ...
    try {
      ...
      await PushService.cancelAlarmNotification(
        _activeAlarm!['id'].toString(),
      );
      await AlarmNativeService.stop();
  ...
  Future<void> _endAlarm() async {
    ...
    try {
      ...
      await PushService.cancelAlarmNotification(alarmId);
      await AlarmNativeService.stop();
      await _loadAll();
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _testNativeAlarmTone() async {
    try {
      await AlarmNativeService.start(
        alarmId: 'local-test',
        title: 'ALARMTEST',
        body: 'Lokaler Test des Daueralarms',
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          content: const Text(
            'Der native Alarm-Service wurde gestartet. Wenn kein Ton zu hören ist, liegt der Fehler im Android-Service oder bei jf_alarm.wav.',
          ),
          action: SnackBarAction(
            label: 'STOPP',
            onPressed: () {
              AlarmNativeService.stop();
            },
          ),
        ),
      );
    } catch (e, st) {
      debugPrint('Alarmtest Fehler: $e');
      ...
      await showDialog<void>(
        context: context,
        builder: (context) => AlertDialog(
          title: const Text('Alarmtest fehlgeschlagen'),
                ...
                children: [
                  Expanded(
                    child: OutlinedButton.icon(
                      onPressed: _testNativeAlarmTone,
                      icon: const Icon(Icons.volume_up),
                      label: const Text('Alarmton testen'),
                    ),
                  ),
                  const SizedBox(width: 10),
                  OutlinedButton.icon(
                    onPressed: AlarmNativeService.stop,
                    ...
                    Container(
                      ...
                      child: const Row(
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              'Alarm aus Push-Benachrichtigung geöffnet. Bitte jetzt Rückmeldung auswählen.',
                              style: TextStyle(
