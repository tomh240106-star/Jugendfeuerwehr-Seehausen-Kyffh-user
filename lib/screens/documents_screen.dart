...
import '../services/duty_plan_web_ocr.dart';
...
class _DocumentsScreenState extends State<DocumentsScreen> {
  ...
  Future<void> _loadRole() async {
    ...
    try {
      ...
      debugPrint('Rolle konnte nicht geladen werden: $e');
    }
  }
  ...
  IconData _iconForFile(String fileName) {
    ...
    if (name.endsWith('.txt')) return Icons.article_outlined;
  ...
  Future<void> _uploadDocument() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Dateiauswahl fehlgeschlagen: $e'),
        ),
      );
      return;
    }
    ...
    if (user == null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Bitte erneut anmelden und den Upload wiederholen.'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      // Kein dart:io und kein File(path), sondern direkter Byte-Upload.
      ...
      final isDutyPlan =
          meta.category.trim().toLowerCase() == 'dienstplan' ||
      ...
      if (uploadedPath != null) {
        try {
          ...
          debugPrint(
            'Aufräumen nach fehlgeschlagenem Upload fehlgeschlagen: '
            '$cleanupError',
          );
        }
      }
  ...
  Future<String> _importDutyPlan({
    ...
    try {
      List<DutyPlanRow> rows;

      if (DutyPlanImportService.isSupported &&
          filePath != null &&
          filePath.isNotEmpty) {
        // Android / iOS: Google ML Kit
        rows = await DutyPlanImportService.extractFromImage(filePath);
      } else if (DutyPlanWebOcr.isSupported) {
        // Web: OCR direkt im Browser.
        rows = await DutyPlanWebOcr.extractFromImageBytes(
          fileBytes,
          fileName,
        );
      } else {
        return 'Dienstplan wurde hochgeladen. Der automatische Import '
      ...
      return 'Dienstplan wurde hochgeladen. Die automatische Übernahme '
          'konnte diesmal nicht abgeschlossen werden: $e';
    }
  }
  ...
  Future<void> _deleteDocument(Map<String, dynamic> document) async {
    ...
    try {
      ...
      try {
        ...
        debugPrint(
          'Dokument gelöscht, Push fehlgeschlagen: $pushError',
        );
      }
  ...
  List<Map<String, dynamic>> get _filteredDocuments {
    ...
    final result = _documents
        .where((document) {
          ...
          final matchesNew =
              !_onlyNew || !_readDocumentIds.contains(id);
      ...
      case 'kategorie':
        result.sort((a, b) {
          final categoryCompare =
              (a['category']?.toString() ?? 'Allgemein')
                  .toLowerCase()
                  .compareTo(
                    (b['category']?.toString() ?? 'Allgemein')
                        .toLowerCase(),
                  );
  ...
  int _categoryCount(String category) {
    ...
    return _documents.where((document) {
      return (document['category']?.toString() ?? 'Allgemein') ==
          category;
    }).length;
  }
                    ...
                    children: [
                      ...
                      if (_isTrainer)
                        IconButton(
                          ...
                          onPressed:
                              _uploading ? null : _uploadDocument,
                          style: IconButton.styleFrom(
                            backgroundColor: Colors.white,
                            foregroundColor: red,
                          ),
...
class _DocumentMetaSheet extends StatefulWidget {
  ...
  @override
  State<_DocumentMetaSheet> createState() =>
      _DocumentMetaSheetState();
}

class _DocumentMetaSheetState
    extends State<_DocumentMetaSheet> {
